...
func NewService(pool *pgxpool.Pool, usersRepo *users.Repo) *Service {
	return &Service{pool: pool, usersRepo: usersRepo}
}
...
func (s *Service) SuspendUser(ctx context.Context, userID, actorID uuid.UUID, note string) error {
	...
	if err := s.usersRepo.SetStatus(ctx, userID, users.StatusSuspended); err != nil {
	...
	return s.audit(ctx, actorID, "user", userID, "user_suspended", note, nil)
}
...
func (s *Service) ReinstateUser(ctx context.Context, userID, actorID uuid.UUID, note string) error {
	if err := s.usersRepo.SetStatus(ctx, userID, users.StatusActive); err != nil {
		if errors.Is(err, users.ErrNotFound) {
			return web.ErrNotFound("user")
		}
		return err
	}
	return s.audit(ctx, actorID, "user", userID, "user_reinstated", note, nil)
}
...
func (s *Service) GrantRole(ctx context.Context, userID, actorID uuid.UUID, role string) error {
	...
	if err := s.usersRepo.GrantRole(ctx, userID, role, actorID); err != nil {
		return err
	}
	return s.audit(ctx, actorID, "user", userID, "role_granted", role, map[string]string{"role": role})
}
...
func (s *Service) CreateCategory(ctx context.Context, actorID uuid.UUID, in CategoryInput) (uuid.UUID, error) {
	...
	_, err := s.pool.Exec(ctx, `
		...
		VALUES ($1, $2, $3, $4, $5, $6, $7)`,
		id, in.Code, strings.TrimSpace(in.Name), in.NameTranslations, strings.TrimSpace(in.Description), in.SortOrder, active)
	if err != nil {
		return uuid.Nil, fmt.Errorf("inserting category: %w", err)
	}
	if err := s.audit(ctx, actorID, "target", id, "category_created", in.Code, nil); err != nil {
		return uuid.Nil, err
	}
	return id, nil
}
...
func (s *Service) UpdateCategory(ctx context.Context, actorID, id uuid.UUID, in CategoryInput) error {
	...
	return s.audit(ctx, actorID, "target", id, "category_updated", "", nil)
}
...
func (s *Service) CreateCriterion(ctx context.Context, actorID, categoryID uuid.UUID, in CriterionInput) (uuid.UUID, error) {
	...
	_, err := s.pool.Exec(ctx, `
		...
		strings.TrimSpace(in.Description), in.Required, in.ScaleMin, in.ScaleMax, in.SortOrder, active)
	if err != nil {
		return uuid.Nil, fmt.Errorf("inserting criterion: %w", err)
	}
	if err := s.audit(ctx, actorID, "target", id, "criterion_created", in.Code, nil); err != nil {
		return uuid.Nil, err
	}
	return id, nil
}
...
func (s *Service) UpdateCriterion(ctx context.Context, actorID, id uuid.UUID, in CriterionInput) error {
	...
	return s.audit(ctx, actorID, "target", id, "criterion_updated", "", nil)
}
